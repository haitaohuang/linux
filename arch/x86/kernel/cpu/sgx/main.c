...
static inline bool sgx_can_reclaim(struct sgx_epc_cgroup* epc_cg)
{
        if (!IS_ENABLED(CONFIG_CGROUP_SGX_EPC))
		return !list_empty(&sgx_global_lru.reclaimable);

	return !sgx_epc_cgroup_lru_empty(epc_cg);
}
...
struct sgx_epc_page *sgx_alloc_epc_page(void *owner, bool reclaim)
{
	...
	for ( ; ; ) {
		...
		sgx_reclaim_epc_pages(SGX_NR_TO_SCAN, false, epc_cg);
		cond_resched();
	}
