 ...
 * have already configured with VM_READ and VM_WRITE at minimum.
 ...
 * to do EAUG immediately in future as indicated by s bit set in arg.flags
 * Return:
 * - 0:		Success.
 * - -errno:	Otherwise.
 */
static long sgx_ioc_enclave_augment_pages(struct sgx_encl *encl, void __user *arg)
{
	...
	if ((params.flags & SGX_PAGE_CET_SSA) && !(secinfo.flags & SGX_SECINFO_SS_REST))
		return -EINVAL;
	...
	if (!vma || vma->vm_private_data != encl ||
		...
		ret = -EACCES;
	} else if (!(params.flags & SGX_PAGE_CET_SSA) &&
	    secinfo.flags & (SGX_SECINFO_SS_FIRST | SGX_SECINFO_SS_REST)) {
		// Want to configure the range as shstk
		if (vma->vm_start == start && vma->vm_end == end)
			vm_flags_set(vma, VM_SHADOW_STACK);
		else
			ret = -EINVAL;
	} else if (vma->vm_flags & VM_SHADOW_STACK) {
		// VMA was configured as shstk. This ioctl should not change.
		ret = -EACCES;
	}
