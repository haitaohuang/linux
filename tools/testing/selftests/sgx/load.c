...
bool encl_load(const char *path, struct encl *encl, unsigned long heap_size, unsigned long edmm_space)
{
...
" If so, remount it executable: mount -o remount,exec /dev\n\n"
	...
	for (encl->encl_size = 4096; encl->encl_size < encl->src_size + edmm_space; )
		encl->encl_size <<= 1;
