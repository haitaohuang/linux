...
test_cg_small_parent=$test_root_cg/test1
test_cg_large=$test_root_cg/test2
# We will only set limit in test1 and run tests in test3
test_cg_small=$test_root_cg/test1/test3
test_cg_larger=$test_root_cg/test4
...
mkdir -p $cg_root/$test_cg_small_parent
mkdir -p $cg_root/$test_cg_large
mkdir -p $cg_root/$test_cg_small
mkdir -p $cg_root/$test_cg_larger
...
if [ $? -ne 0 ]; then
    ...
    echo "SKIP: SGX cgroup tests require misc and memory cgroups enabled."
    clean_up
    exit $ksft_skip
fi
...
if [ $? -eq 0 ]; then
    echo "# Fail on small EPC limit, not expecting any test passes."
    clean_up
    exit 1
else
...
echo "# PASSED small EPC limit test."
...
if wait_and_detect_for_any $PROCESS_SUCCESS "$pids"; then
    echo "# PASSED large EPC limit positive testing."
else
    echo "# Failed on large EPC limit positive testing, no test passes."
    clean_up
    exit 1
fi
...
if wait_and_detect_for_any $PROCESS_FAILURE "$pids"; then
    echo "# PASSED large EPC limit negative testing."
else
    echo "# Failed on large EPC limit negative testing, no test fails."
    clean_up
    exit 1
fi
...
if wait_and_detect_for_any $PROCESS_FAILURE "$pids"; then
    echo "# Failed on larger EPC limit, at least one test fails."
    clean_up
    exit 1
else
    echo "# PASSED larger EPC limit tests."
fi
...
echo "# PASSED larger EPC limit test with a process randomly killed."
...
if wait_and_detect_for_any $PROCESS_SUCCESS "$pids"; then
    ...
    swapon -a
    echo "# PASSED large EPC limit tests with memcontrol."
fi
...
if [ "$epc_usage" -ne 0 ]; then
    echo "# Failed: Final usage is $epc_usage, not 0."
else
